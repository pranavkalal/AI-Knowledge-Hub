...
def _tables_pdfplumber(pdf_path: str) -> Dict[int, List[List[str]]]:
    """
    Extract tables per page as a list of rows (strings). Returns {page_index: [table1_rows, table2_rows, ...]}.
    """
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for i, page in enumerate(pdf.pages):
            page_tables = []
            try:
                # 'extract_tables' auto chooses strategies; reasonable default
...
def _markdown_table(rows: List[List[str]]) -> str:
    ...
    if not rows:
        return ""
    # Decide if first row is header: heuristic = no more than 2 empties & not all numeric
    header = rows[0]
...
def _extract_image_ocr_regions(doc: fitz.Document, page_idx: int, max_dim: int = 200):
    """
    Optionally OCR embedded images on a page that are large enough to contain text.
...
def parse_pdf(pdf_path: str, extra_meta: Optional[Dict[str, str]] = None) -> ParsedPDF:
    ...
    tables = _tables_pdfplumber(pdf_path)  # {page_idx: [table_rows, ...]}
