    ...
    const handleSend = async (text: string) => {
        ...
        try {
            ...
            const aiMsg: Message = {
                ...
                citations: response.citations.map(c => c.sid) // Using SID for now (e.g., S1, S2)
            };
            ...
            const errorMsg: Message = {
                id: (Date.now() + 1).toString(),
                role: "assistant",
                content: "Sorry, I encountered an error connecting to the Knowledge Hub. Please try again.",
                error: true
            };
                ...
                <div className="space-y-6 pb-4">
                                ...
                                {msg.citations && msg.citations.length > 0 && (
                                    <div className="flex flex-wrap gap-2 mt-2 pt-2 border-t border-slate-100">
                                        ...
                                        {msg.citations.map((cite) => (
                                            <button
                                                key={cite}
                                                onClick={() => onCitationClick("sample-doc-id")} // TODO: Map SID to DocID
                    ...
                    {isLoading && (
                        <div className="flex gap-3">
                            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#009B77] text-white">
                                <Bot className="h-5 w-5" />
                            </div>
                            <div className="flex items-center gap-1 rounded-lg bg-white p-4 shadow-sm border border-slate-100">
